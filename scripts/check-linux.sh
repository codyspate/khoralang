...
# Where this script lives, so it can find the half that runs inside.
...
# **One `wsl` invocation for all of it, and that is a fix rather than tidying.**
# This was two more calls from here, and `wsl.exe` can return before its child
# has finished writing: the "sequential" steps overlapped, both sharing
# `CARGO_TARGET_DIR`, so the second cargo could start against a tree the first
# was still linking into. About one run in four died with cargo's exit 101 and
# an interleaved log that said nothing about why — unexplained for two
# sessions, then reproduced on the fourth attempt of eight once the output
# stopped being discarded.
#
# The Linux side is a file rather than a quoted string for the same reason it
# is one call: `bash -lc "..."` made every backtick in a comment a command
# substitution, and one of them ran a design document.
wsl -e bash -lc "cd '$inside' && sh scripts/linux-inner.sh '$TARGET' '$REPEATS'"
