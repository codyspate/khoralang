...
step 'the checks that need nothing built'
# **First, because they take a second and the suite takes twenty minutes.**
#
# Every check below reads text: source, documents, the rules file. None needs a
# compiler, a runtime archive or a target directory -- so running them after the
# suite means a missing line in `backend-rules.txt` is discovered twenty-five
# minutes into a CI job, and on the platform that is slowest to get there. That
# happened three runs in a row, and each turnaround was most of an hour for a
# failure a shell could have reported before the first crate compiled.
#
# They stay in one step so the ordering cannot drift back: anything that needs
# only `sh` belongs above `cargo`, and the way to keep that true is to have one
# place where such a thing goes.
