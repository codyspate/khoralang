# Developing the Khora VS Code extension

Notes for people working on the extension or the compiler. The user-facing
page is `README.md`, which is what the Marketplace and VS Code show.
...
decides which toolchain to use itself.

| Feature | From |
| --- | --- |
| Errors and warnings | `khora_types::diagnostics` and `khora_lint::findings` |
| Hover | the checker's `BodyTypes` |
| Definition, references, rename | `khora_hir::resolve_path`, and `Body` for locals |
| Completion | the checker's types and signature keys |
| Semantic highlighting | the resolver |
| Formatting | `khora_fmt` |
| Add a missing import | `khora_lsp::imports` |
| Highlighting, comments, brackets | the TextMate grammar here |

`package.json` scopes format on save under `[khora]`, so it never changes
the setting for another language.
