...
///
/// The walk lives in `khora_syntax::doc`, because `derive(Decode)` reads the
/// same block into a field's schema description: one reader, so the rendered
/// page and the schema cannot disagree about where a comment ends.
fn doc_of(node: &SyntaxNode) -> Vec<String> {
    khora_syntax::doc::doc_comment(node)
}
