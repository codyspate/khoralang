...
/// **An `impl` on an effect, which is where every capability constructor
/// lives.**
///
/// `exported_here` matched an exported type or an exported trait, and a
/// capability is neither — so `impl FsRead { pub fn real() }` was dropped whole
/// and `FsRead::real` appeared nowhere in the reference. The same for
/// `Clock::real`, `Env::real`, `Random::seeded` and `HttpClient::real`: every
/// capability constructor in `std`, which is the one function somebody needs
/// before they can write a program that does anything at all.
///
/// They were findable in cookbook code blocks, by a reader who already knew the
/// name to search for.
#[test]
fn an_effects_methods_are_documented() {
    let page = markdown(&module(
        "module std::thing;\n\n//! What it is.\n\n\
         /// Reaching outside.\npub effect Out {\n  go: () -> Int,\n}\n\n\
         impl Out {\n  /// The real one.\n  pub fn real() -> Out {\n    \
         handler for Out { go: fn () => 1 }\n  }\n}\n",
    ));
    assert!(page.contains("\n## Effects\n"), "{page}");
    assert!(page.contains("\n## Methods\n"), "{page}");
    assert!(page.contains("pub fn real() -> Out"), "{page}");
...
/// a file happened to write an `impl` for it. Widening it to effects must not
/// widen it to everything.
#[test]
fn an_impl_on_a_foreign_name_is_still_dropped() {
    let page = markdown(&module(
        "module std::thing;\n\n//! What it is.\n\n\
         impl Elsewhere {\n  /// Not ours.\n  pub fn f() -> Int { 1 }\n}\n",
    ));
