...
impl<'ctx> Backend<'ctx> {
    ...
    pub(super) fn emit_c_main(&mut self, entry: Option<&str>) {
        let Some(entry) = entry else {
            self.error(
                "this package has no `main` function, so there is no program to build. A \
                 library has none by design: `khora build --lib` makes a C shared library \
                 from its `pub extern fn`s. A program needs `pub fn main()` in \
                 `src/main.kh`",
