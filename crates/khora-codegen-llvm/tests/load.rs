...
fn run(name: &str, items: &str, body: &str) -> String {
    let main = format!(
        r#"module demo::main;
import std::core::{{
  Changed, Channel, Eq, Fiber, Fibers, List, Nursery, Option, Result, Share, Shared, Show,
...
fn a_server_under_more_load_than_it_can_serve_answers_everybody() {
    ...
    let main = format!(
        r#"module demo::main;
import std::core::{{
  Nursery, Option, Result, Share, Shared, SharedFn, Show, attempt, bounded_nursery, print
}};
