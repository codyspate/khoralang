...
| 14.20b | **Delete `[fmt] explicit-semicolons`** | It cannot do anything: `docs/project.md` §14 says every statement must end in a semicolon and the parser agrees, so `false` is not a thing a project can choose. Removing a documented manifest key is a format change, small but not this commit’s to make. Three example manifests and §4.1 carry it |
