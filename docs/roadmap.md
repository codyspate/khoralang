...
| 14.20 | **Release tooling** | Which members changed, bump them and their dependents, write the notes. `publish = true` already exists per package; this is the part that decides what to do with it. **`docs/design/releasing.md` narrows it to four questions and recommends an answer to each** — the version policy, the pre-1.0 note rule and the draft-then-publish mechanics are all already decided elsewhere, so what is open is: one version or one per member (recommend lockstep, because with no registry a repository tag already names one state of every member), whether the tool tags (recommend never), where notes come from (recommend a skeleton it refuses to call finished), and what counts as a change to a member (recommend 14.16’s selection unchanged). **Blocked on those four, not on engineering** |
