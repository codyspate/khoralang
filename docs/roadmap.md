...
| 13.11 | Public surface audit | **Audited; one decision open.** 390 items in `std`, of which 94 said nothing — all written now and held by a test. The open one is a language question: **`export` means nothing inside an `impl`**, so 317 `std` methods that never asked to be public are promises, and `packages/postgres` writes the keyword while `std` does not. `docs/design/std-surface.md` has the options |
