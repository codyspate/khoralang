...
| 14.33 | **The runtime archive changes under a parallel test run** | Found by 14.17’s cache, errata 51. `khora-codegen-llvm`’s harness runs `cargo build -p khora-rt` from inside a test so an edit to the runtime cannot leave a stale archive; that build resolves a different feature set from the one the test run did, so cargo relinks the staticlib and `target/debug/khora_rt.lib` flips between two files **while other tests are running**. Two `khora build` invocations seconds apart therefore link different runtimes. The cache tests pin `KHORA_RT_LIB` to a copy and are fine; the repository is not, and nothing else had ever noticed because nothing else compares two builds for identity |
