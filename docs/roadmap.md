...
| 13.23 | ✅ Performance at scale | **The harness was not the ceiling; it was the number.** `bench/load.py` reported one connection's rate multiplied by the number of connections — one process per connection, each timing itself, divided by the duration it was *asked* for — so every figure this repository published was two to twelve times too high and no ladder could ever flatten. Errata 77. `bench/loadgen.rs` replaces it: a few threads driving many non-blocking connections, whose own rate stops changing when given more of the machine, and `bench/measure.py` checks all four conditions from `/docs/performance/` and prints what failed instead of a number. Measured: `std::net::http` **174,201 req/s**, p50 161us, p99 554us, peak RSS **8.4 MB** — a little under Go's `net/http` on rate, a third under Kestrel, about 4× Node, where this repository had claimed 10× Go and 6× Kestrel; and against Go's 21.8 MB, Node's 86.8, Kestrel's 240 and the JDK's 699, which is the column nobody had instrumented and the one Khora leads. Threads beat the scheduler by 23% at 32 connections, which re-establishes the `fibers.md` decision on evidence that measures what it claims. A second machine would still turn `floor`'s lower bound into a figure |
...
| 14.12 | **Published, not built by hand** | **Half done: there is a release, and it is not the marketplace.** `.github/workflows/extension.yml` packages the `.vsix` on a `vscode-v*` tag and publishes it on a release of its own, so installation is a download and one `code --install-extension` rather than a checkout and a build. Separate from `release.yml` deliberately: the extension has no compiler in it, versions in its own `package.json`, and changes for its own reasons — rustup and rust-analyzer split the same way. Still open: the marketplace and OpenVSX, which need publisher credentials, and an extension that finds or fetches a *matching* server rather than trusting whatever `khora` is on `PATH`. Along the way, `install.ps1` — the hand-rolled packager this row was written about — was found to omit `src/extension.js` and `node_modules` entirely, so every `.vsix` it ever produced installed cleanly and had no language client in it. It now shells out to `vsce` and verifies the archive; the workflow verifies the same two things, because the failure is invisible from the outside |
...
| 14.34 | ✅ **`khora run`, and paths you do not have to type** | Asked for directly. `khora run` compiles and starts the program the way `cargo run` does, forwarding the program’s exit status and passing anything after `--` to it; the build goes through 14.17’s cache, so a second run of an unchanged program starts in about a third of a second. The task runner moved to `khora task` — `run` is the first command a newcomer types and every other toolchain means one thing by it. Every path argument is optional. Two bugs fell out: `khora check` and `khora check .` were **different commands** (an empty list reached `collect_sources`, which substitutes `.` there and nowhere else, so the bare form walked the whole tree as one compilation while the explicit form fanned out over eight members), and `khora build` at a workspace root silently built whichever member’s source contained `fn main(` first |
...
contents that are no longer there. Git calls those entries *racily clean*. The
memo is now believed only when it was recorded strictly after the file it
describes was last written. The unit test that says so was written to check the
claim and failed on its first run, which is the argument for writing the test
that tries to break your own soundness paragraph.
...
| 14.36 | ✅ **`std::schema` is the one path for untrusted data** | Done, under #170. `derive(Decode)` writes a type's schema from its declaration, carrying each field's `///` as its description, and `struct({ .. })` is a compiler-known call the checker types by the record literal's own rules, so `struct2`..`struct5` and the argument that the literal "cannot be typed" are gone. `std::json`'s `FromJson`/`ToJson`, `std::config`'s four readers and `std::ai::Extract` are deleted; a JSON body, the environment, the command line, a query string, a database row and a model's answer each hand a `Raw` to the same schema, which reports every problem with a path. `Shape::to_json_schema` is what a model is prompted with and what an API is documented by. `docs/design/schema.md` is the library as it stands; `docs/design/schema-derive.md` is the decision record. Errata 76 |
