...
two, and database failure under network loss -- which was scored by writing the
missing tests, and they found erratum 80. The eighth and ninth are the two
...
**Last read, rather than last counted: 2026-09-22**, against `a2593dd`
(`v0.2.0` plus 53 commits). The seven open items were re-checked against the
tree and none closed; two notes had drifted and are rewritten -- §1's 12.9
remainder, which provenance attestation discharged a fortnight before the last
read, and §2's soak item, which said macOS was exercised nowhere while
`runtime.yml` runs the adversarial tests there on every runtime push. Of the
ticked items, §19's defect table still called the nursery-cancel hang
documented rather than fixed, and it has been fixed since `08e941b`; §2's
nursery item claimed sibling cancellation without the adoption-order caveat its
own documentation carries. The
summary table below had drifted from the boxes in fifteen of twenty-one rows
and is now recounted from them.

**The read before that, 2026-09-03.** Every open item was
...
Counted from the boxes on 2026-09-22; the rows sum to the line above.

| Section | Done |
| --- | --- |
| 1. Language and compiler correctness | 18 / 19 |
| 2. Runtime soundness and structured concurrency | 14 / 16 |
| 3. Resource, database and cancellation semantics | 6 / 6 |
| 4. HTTP, overload and server behavior | 10 / 10 |
| 5. Observability | 6 / 7 |
| 6. Database ecosystem proof | 7 / 7 |
| 7. Cross-compilation and deployment | 11 / 11 |
| 8. FFI and C interoperability | 8 / 8 |
| 9. Traps, debugging and production diagnosis | 7 / 7 |
| 10. Compiler performance and scale | 6 / 6 |
| 11. Tooling and editor experience | 10 / 10 |
| 12. Installation, toolchains and release artifacts | 9 / 9 |
| 13. Package ecosystem | 7 / 7 |
| 14. Supply chain and security | 7 / 7 |
| 15. Compatibility, governance and contribution policy | 8 / 9 |
...
and found five real defects, all of which are now fixed; what it did not cover is
...
concentrated in the same place: native I/O backends beyond Linux's `epoll`
(§2), the long soak on macOS (§2), a reference service whose trace has
parent/child spans (§5), and a way for `std` to mark an item unsettled (§15).
The formal `unsafe` inventory, cancellation cleanup for all four resource kinds
(§3) and compiler performance at scale (§10) were on this list and are done. The largest reference application was about 460 lines, which
...
- [ ] Phase 12 is complete, including all implementation work that remains in its entries rather than only the currently landed subset. **Left:** two roadmap entries are partial; the note said three until 2026-09-22. Both of #140's named remainders are closed: `khora run src/a.kh` inside a package refuses and names `src/bin/` rather than silently running the package's program, and an unused *type* import warns like any other. What is open is in the roadmap's own headings. **12.2 cross-compilation is "step one done"**, re-checked on 2026-09-22: `KHORA_TARGET=aarch64-unknown-linux-gnu khora build` emits the object and stops at the link, naming the missing sysroot. **12.4 debug information has line tables and locals but not heap layout** -- `crates/khora-codegen-llvm/src/debug.rs` still describes a boxed value as a pointer rather than a struct a debugger can walk. **12.9 is discharged and its roadmap heading has not caught up:** it says "the SBOM, not the signature", and the signature it was waiting for is the `actions/attest-build-provenance` attestation `release.yml` has carried since `99a5811` (2026-09-02), which §14 already ticks.
...
- [x] Type inference and lowering have regression coverage for closures, generics, traits, effect rows, handlers, capabilities, higher-kinded types, ADTs, pattern matching and annotations. **Done:** 2,639 tests in the Windows CI run of 2026-09-22 (2,646 on Linux), up from 2,107 at the 2026-09-03 read; `khora-types/tests` and `khora-codegen-llvm/tests` carry a file per feature.
...
- [x] Nursery semantics are complete: a failing child has the documented effect on siblings and the parent, with typed failure behavior tested. **Done:** #139 made the first failure cancel the siblings, every child is waited for, and the nursery raises `ChildFailed` -- **but the cancelling half is only as prompt as adoption order allows**, and the note said it unconditionally until 2026-09-22. Re-measured on `a2593dd`: eleven 400 ms siblings and a failing child adopted last, both backends, three runs each -- all eleven ran to completion and the nursery returned at about 400 ms. The tick stands because the item asks for the *documented* effect and `/docs/reference/concurrency/` and `/docs/limitations/` both document exactly this; roadmap 16.7 owns the repair.
- [x] No language-visible behavior depends on a fiber staying on one OS thread unless the program explicitly enters a documented thread-affine FFI boundary. **Done:** `khora-rt/src/migration.rs` runs the scheduler backend and checks the three things `current.rs` holds -- the fiber's identity, its cancellation flag, and the current span -- on every turn of a fiber that changes worker. Those three because each was a thread-local at some point in this runtime's life, and the identity one was a live bug rather than a hypothetical: `Shared::update`'s re-entry check read the wrong fiber's id and killed correct programs.

  **The tests retry, and that is the finding.** Written as one run with an assertion at the end they passed on the first attempt and reported *no fiber ever changed worker* on the second -- migration happens when a worker steals, which is a race rather than a promise, so both outcomes were meaningless. Each test now runs the whole scheduler until it observes a migration and fails only if it never sees one, which removes the flaky *pass* rather than the flaky failure.
...
- [x] Every runnable `Task` has exactly one owner at every instant; wake tokens or backend state never create a second owner. **Done:** the invariant is written at `scheduler::wake`, which is the one place it can be broken -- a wake arrives from outside, knows nothing about what the fiber is doing, and any number of them can arrive for one fiber from any number of threads. Two claims stop it and the note says why neither alone is enough: `state.wake()` is a compare-exchange from `WAITING` that exactly one caller wins, and `parked.remove` hands the task to exactly one caller, both under the lock the worker parks with. A `debug_assert` guards the map. `an_avalanche_of_wakes_resumes_a_fiber_once` is the test at the scheduler level: fibers park repeatedly while four threads wake every one of them twice over, and the assertion is that resumptions equal parks -- a task claimed twice would resume twice from one park -- with the audit checked for a task left in transit or counted twice. `concurrent_wakers_produce_exactly_one_owner` in `wait.rs` covers the first claim and is one of the modules `scripts/tsan.sh` runs, which is as far as ThreadSanitizer reaches here: it dies on a fiber's guard page, so the scheduler's own tests are outside it, and the script says so.
...
- [x] Each inventory entry names the invariant that makes it sound and the test or argument that protects the invariant. **Done, and doing it found the failure mode.** The gap was never that entries lacked a test name -- the load-bearing ones had them -- it was that nothing checked the names still meant anything. `soundness.md` cited `a_fiber_keeps_its_identity_across_workers` as the protection for thread-affinity under migration. The test existed, so a check for the name would have passed, and **it never asserted that a fiber changed worker** -- so on a run where nothing migrated it proved nothing while reading like a guarantee. It is rewritten, under the same name so the citation still resolves, and now retries until it observes a migration. `scripts/check-cited-tests.sh` is a gate step covering the next stage of the same decay: every backticked `snake_case` name of three or more words in `soundness.md` and `scheduler.md` must appear in `crates/`. It found one dangling name on its first run -- `build_in_bounds_gep`, which turned out to be inkwell's and is why the check asks for a mention rather than a definition. It deliberately does not claim to check that a test still asserts what the document says; no script can, and pretending otherwise would be worse than the gap.
...
- [x] File, socket, TLS and process resources have cancellation tests that prove cleanup rather than merely absence of a crash. **Done, all four.** Files delete the file a cancelled fiber was reading, which Windows refuses while a handle is open. Sockets bind the port again and watch the peer see the connection close. **TLS** is `tls_cancel.rs`: a session held through a region, a fiber cancelled while holding it, and a Python client that must see the connection close promptly rather than wait out its own timeout. **Process** is `process_cancel.rs`, and it found that there are two questions rather than one -- a fiber cancelled *before* the call never starts the child at all, and a fiber cancelled *during* the wait lets it run to completion and reaps it. Both are pinned, and the second needed a cancellation from outside, which a nursery delivers and a `khora_cancel` in the fiber's own body cannot.

  **Every one of these was checked by deleting the release and watching it fail**, which is the check the entry demanded and is not a formality: the TLS test passed with the `acquire` removed on the first attempt, because the server process exited and the operating system closed the socket, so the client saw a close either way. It holds the process open past the client's timeout now, and fails at 8.009s without the release. `net_cancel.rs` had learned the same lesson and the note in it is what prompted re-checking this one.

  Two things found on the way and worth keeping: a `TlsConnection` cannot cross a fiber boundary -- the compiler refuses, because it can be written and two fibers writing one session is a race -- so the handshake has to happen on the fiber that gets cancelled rather than being handed to it. And a nursery notices a child's failure in *adoption order*, so a test that adopts the fiber it wants cancelled first never cancels anything; both new tests say so where the order is written.
...
- [ ] A reference service demonstrates an incoming HTTP trace flowing through application work, spawned fibers and database operations with correct parent/child relationships. **Left:** the third of it that is about parent/child, and it is blocked on `std::trace` rather than on the example. `examples/ledger_service` takes a `Tracer` through its handlers and wraps each database operation in `around_result` -- `entries.create`, `entries.list` -- so a request does flow through application work into the database with a span around it. **The effect can express the relationship now, and the example does not use it yet.** That is the whole of what is left. `std::trace::current` is the span a fiber is inside, kept by the runtime per fiber and inherited by a child at the moment it is spawned; `around` installs and restores it on every path out including a cancellation, and a handler's `start` takes the trace id, the sampling decision and the parent from it. Five tests in `khora-codegen-llvm/tests/tracing.rs` cover nesting, siblings, two roots, a failed span restoring the one around it, and a spawned fiber staying in its spawner's trace. `std::log` correlates by itself as a result -- a line logged inside a span carries `trace_id` and `span_id` with nothing threaded through a signature. What the example still needs is a request-level span per handler and some concurrent work under it: it installs `Tracer::none()` and spawns nothing, so it would demonstrate the plumbing rather than exercise it. That is an example to write against a running Postgres, not a language question any more. Re-checked 2026-09-22: no commit since `v0.2.0` touched `examples/ledger_service`, which still installs `Tracer::none()` and spawns nothing.
- [x] Logging guidance explains how logs correlate with traces and fiber/request context. **Done:** the blocker was a decision rather than a page -- there was no logging capability for logs to correlate *with*, and no way to write to standard error at all. `std::log` is both: `eprint` as the primitive, and a `Log` effect over it emitting one JSON object per line with `timestamp`, `level` and `message` in a fixed order, attributes as typed fields, and five levels filtered by the handler rather than the caller. It is a capability for the reason everything reaching outside is one, which also makes it testable -- a test installs a logger that collects into a list, with no global to reset. `/docs/cookbook/logging/` covers all of it, including correlation: attributes are `std::trace`'s `Attribute`, so a line carries `trace_id` and `span_id` in the field names OpenTelemetry expects. **And nothing has to be passed in.** The handler asks `std::trace::current` what the fiber is inside and writes `trace_id` and `span_id` itself, or writes neither field when there is no span rather than thirty-two zeros a collector would group every unparented line under. The page said the opposite while it was true, which is the second half of what these entries are for.
...
- [x] Connection and transaction failure behavior is tested under cancellation and network loss. **Done, and writing the missing half found a bug.** Cancellation was already covered thoroughly. Network loss now is: a connection lost during the body, during the `COMMIT`, and before `BEGIN`, each asserting that the handler is told `broken` so the connection is not handed to the next borrower. **The commit case was wrong** -- a commit that failed because the socket had died was treated exactly like one the engine refused, so a connection whose `COMMIT` may or may not have arrived went back to the pool as healthy. Erratum 80. `a_refused_commit_leaves_the_connection_alone` is the companion that stops the fix becoming "always call `broken`", which would empty a pool on every constraint violation. The commit fix was checked by reverting it and watching the test fail.
...
- [x] Thread-affine foreign libraries are tested against fiber migration rules. **Done:** the second half of `migration.rs` stands up a pretend thread-affine library -- one that records the thread it was initialised on, the way an OpenGL context, a COM apartment or a single-threaded SQLite does -- and demonstrates the rule `/docs/reference/ffi/` states rather than restating it. Three claims: a run of foreign calls with no suspension between them stays on one thread, which is what makes such a library usable at all and is the observable form of *an ordinary foreign call cannot secretly suspend Khora*; a handle held across a suspension **is** used from the wrong thread, asserted to happen because the point is to demonstrate the hazard rather than hope it is absent; and one `blocking` call runs on a single pool thread start to finish, which is what makes the documented boundary worth having.
...
- [x] At least one external-language integration test (for example Python, Node or a small C host) validates the public C surface. **Done:** `tests/exporting.rs` compiles and runs a C host against the exported surface. **The other direction exists too, since `b1deb3c`:** `[build] link` and `link-search` in the root manifest put a native archive on the link line, so an `extern fn` can be satisfied by a C library rather than only declared. `khora-cli/tests/check.rs`'s `only_the_root_package_may_link_a_native_library` builds a C archive, shows the link failing without the key and a program calling into it with the key, and passed locally and on `windows-latest` on 2026-09-22.
...
- [x] A tagged public release exists with an explicit semantic version such as `0.1.0`. **Done:** `v0.1.0`, published 2026-09-03 from `a2f03d1`, with toolchains for all three supported triples, their checksums and a CycloneDX bill of materials. Each artifact was unpacked on a clean runner and used to compile and run a program with no `KHORA_STD` or `KHORA_RT_LIB` set before the release left draft — which is the check that caught the packaging step still looking for a binary at `src/main`, three platforms red on a toolchain that was fine.
...
- [ ] `std` can say "not settled yet", so that tagging does not freeze the whole library at once. **Left:** the mechanism; the argument is made in `docs/design/std-admission.md` § What this does not decide, and should. There is no `unstable`, no `preview`, no `experimental` marker anywhere in the front end, so every item in `std` becomes a promise on the day of the tag -- on a library whose oldest line is weeks old. That makes each admission question binary, promise it forever or exile it, when the honest answer for `Router`, the retry drivers and the `schema` combinators is "probably right, ask again in a year". Two shapes are sketched there: `std::preview::*` as a namespace items graduate out of, which needs no package machinery, or first-party bundled packages versioned separately, which needs resolution without a registry round-trip that `khora-pkg` does not have. **This is a gate item rather than a roadmap one because it is not a feature -- it is what the release is promising, which is the one thing a tag cannot be walked back.** Re-checked 2026-09-22: `9382576` reserved the word `unstable` (`RESERVED_WORDS` in `crates/khora-syntax/src/kind.rs`), which keeps the spelling available and builds no mechanism -- the parser refuses the word as a name and nothing reads it as a marker. Still open as described.
...
- [x] Release documentation is versioned and remains addressable after newer releases ship. **Done:** the machinery is built and serving two trees — `website/versions.mjs` is the one list both `sync-docs.mjs` and `astro.config.mjs` read, every page lives under `/docs/next/` rather than at the site root, and `/docs/` follows the newest stable entry by computation rather than by somebody remembering to edit it. `/docs/v0.1/` is the tree `v0.1.0` shipped and stays addressable when v0.2 ships beside it. Two decisions recorded here were reversed on the way: that versioned paths should wait until two versions existed, and then that no stable tree should exist before v1. The second was right only while `next` was the only compiler anybody could install, and stopped being right within the hour `v0.1.0` published — a front door redirecting every reader to a tree banner-marked *this describes the unreleased compiler* is a front door with nothing behind it. `docs/design/docs-urls.md` carries the contract and both reversals.
...
- [x] `/docs/<version>/` resolves pinned documentation for supported historical releases. **Done:** `/docs/v0.1/` serves the 77 pages `v0.1.0` shipped, and `/docs/next/` the ones being written; `/docs/` follows the newest stable entry, which is now `v0.1` rather than `next`. No release is *historical* yet — v0.1 is the current one — and the first that becomes so needs no new work, since adding a tree is a directory and a line. Sections are per release allowed to break: the major from 1.0, the minor before it, because that is where semver puts the breaking change. Building the second tree is also what found the bug: Astro slugifies a path segment and a slugifier eats dots, so `v0.1` was being served at `/docs/v01/` while every link and redirect pointed at `/docs/v0.1/`. One entry in the list had never exercised the disagreement it was written to prevent.
...
- [x] Installation failures, confusing diagnostics, undiscoverable APIs and documentation gaps found in that exercise are addressed or explicitly documented before release. **Done, and this is where the exercise earned its place.** Five defects found, all five fixed -- four before `v0.2.0` and the fifth after it:
...
- [x] Baseline/compiler tests, runtime stress, HTTP conformance, examples, docs links/snippets and package-resolution tests pass for the exact release candidate. **Done:** `scripts/baseline.sh`: 2,639–2,646 tests depending on platform (CI run 35804497363, all six jobs green), conformance, corpus, packages, cache and the Linux runtime through WSL2.
