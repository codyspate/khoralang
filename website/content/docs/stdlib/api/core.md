...
naming it resolves against; nothing resolves *to* it. It is the solver's
bottom — the type a `raise` already has.
...
What a fiber ended as, when the asking must not unwind the asker.

`Fiber::outcome` produces one. `Answered` carries what the fiber computed;
`Stopped` says a cancellation reached it and carries nothing, because a
fiber that was stopped has no answer to carry.

**`Stopped` is not a failure.** A child that *failed* raises on the fiber's
own row and never appears here — which is what keeps a failure the
caller's to interpret rather than one arm among two.
...
**Why `new` takes no seed element.** `Array::new` needs a value to fill
with, and there is no value of an arbitrary `A` to be had: `Vector::new()`
has nothing in its hands, and neither has `with_capacity`, whose whole
purpose is to allocate *before* there is anything to put in it.

A vector of a hundred integers is two live objects — the vector and its
array — at that length and at every other. Allocating per element is
...
not. A database driver is the shape this is for.
...
The other half of `map_err`: a reader who has met one goes looking for
this and finds a `match`.
...
`sort_by`, which takes the comparison.
...
loop {
  ...
  if Fiber::finished(server) {
    ...
    // It ended on its own, which for a listener is never good news.
    Fiber::join(server)!;
    break
  };
  if Shared::get(stop) { break }
...
**It reports that the fiber was stopped, not what it had computed.** A
`true` says a cancellation reached the fiber and the fiber gave up on it;
it says nothing about how far the work got, and there is no way here to
...
A `false` is a fact about the instant it was asked, exactly as it is for
`finished`, so this belongs in a loop that sleeps between looks. Asking
does not consume the handle: a supervisor asks and keeps supervising.

**A detached fiber answers `false`, whatever happened to it.**
`Fiber::detach` releases the shared state this reads, so after a detach
there is nothing left to ask and the honest answer is the one a released
handle always gives. `Fiber::finished` has the same shape for the same
reason. Ask before detaching, or keep the handle.

#### outcome

```khora
pub fn outcome(self) -> Outcome<A> raises 'er
```
...
tolerates a stop had neither call, and this is that one: the stop comes
back as a value.

```khora
let f = Fiber::spawn(fn () => tally(rows));
Fiber::cancel(f);
...
**The ceremony scales with the fiber's own row, which is the rule `join`
follows.** On a `Fiber<Int, {}>` this is infallible and needs no `!`. On
a fallible fiber it raises `'er`, and **a child that *failed* still comes
out by name** — only a child *cancellation* comes back as a value, so the
caller who decides what a failure means keeps deciding it.

```khora
match Fiber::outcome(worker)! catch {
  DbError::Disconnected(_why) => Outcome::Stopped,
} {
  Outcome::Answered(rows) => rows,
  Outcome::Stopped => List::Nil,
}
```
...
Takes the lock for the read, so it never observes a half-written value.
The answer is a retain rather than a deep copy, and it is a snapshot: a
concurrent `set` or `update` may land the moment after it returns.
...
Takes the lock, so it does not interleave with an `update` in progress.
**It does not read first**, so `set(get(cell) + 1)` is two steps and two
fibers running it can lose one of the increments — that is what `update`
is for.
...
421 ms later. The group does collapse and every child is waited for.
Promptness is not something to build on yet. Roadmap 16.7.
...
not promise tail calls and a frame per element dies on a long list.
...
**And the body may name the nursery**:
