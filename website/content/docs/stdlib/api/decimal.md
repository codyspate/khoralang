...
for the significand stops the program, the same as anywhere else. It does
not saturate: a saturated quotient is in range, printed to the right number
of places, and wrong.
...
and the point is twenty thousand places to its left. Nothing in `std` or in
the runtime caps it, so no operation refuses one -- but `Show` writes a
character per place, and the arithmetic aligns operands to the larger of
two scales before it works, so a wide scale is paid for in both. A scale
arriving from input rather than from a literal is worth a bound of the
caller's own choosing; there is no obvious one to impose here, and imposing
a wrong one would silently change values that are correct today.
