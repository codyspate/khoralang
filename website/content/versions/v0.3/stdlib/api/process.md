...
**A process cannot be started from Khora alone**: `system` and `popen` are
all ISO C offers, and `CreateProcess` takes a `STARTUPINFO` while
`posix_spawn` wants a `posix_spawn_file_actions_t` — neither reachable from
Khora, because no struct crosses the C ABI. They are reachable from the
*runtime*, where Rust's
...
```khora
import std::process::{Process, checked_output};

fn revision() -> String with { process: Process } raises ProcessError {
  checked_output("git", ["rev-parse", "--short", "HEAD"])!
}
```

Each argument crosses whole, so nothing here is quoted and nothing is
parsed on the way: a path with a space in it is one argument with a space
in it. `process.shell` is the other thing, and is named for what it does.
